...
from models.LSTM_MLP_Ensemble import LSTM_MLP_Ensemble
...
def evaluate(model, test_loader, threshold=0.5):
    ...
    with torch.no_grad():
        for test_batch in test_loader:
            ...
            y_score_i, encoded_sent = model(text, text_len, topic_embedding.float())
...
parser = argparse.ArgumentParser(description='Testing LF2')
parser.add_argument('--model_name', type=str, default='LF2', help='name of trained model')
...
mlp_args = {}
# this is actually the topic embedding dimensionality, not no. of topics, but lets just call it that for uniformity
mlp_args['num_topics'] = args.topic_dim
...
for test_file in test_files:
    ...
    test_data = CommentsDatasetv3(csv_file=test_csv, vocab=vocab, delimiter=' ')
    test_loader = DataLoader(test_data, batch_size=args.batch_size, shuffle=False)

    best_model = LSTM_MLP_Ensemble(lstm_args=lstm_args,

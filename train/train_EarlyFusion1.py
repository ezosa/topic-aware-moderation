...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for train_batch in train_loader:
            ...
            pred = model(text, text_len, topics.float()).to(device)
    ...
    save_metrics(save_path + model_name + '_metrics.pt', train_loss_list, valid_loss_list,
                 global_steps_list, log_file)
    print('*'*5, 'Finished Training!', '*'*5, file=log_file)


parser = argparse.ArgumentParser(description='Early Fusion 1: text + topic vectors')
...
model_name = "EF1_"+str(args.num_topics)+"topicVectors_" + str(args.topic_dim) + "topicDim_" + \
...
print("-"*10, "Model name:", "-"*10, model_name, file=log_file)

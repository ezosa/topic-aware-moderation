...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for train_batch in train_loader:
            ...
            topic_emb = train_batch['topic_embedding']
            topic_emb = torch.stack(topic_emb, dim=1).to(device)
            pred = model(topics.float(), topic_emb.float())
            ...
            if global_step % eval_every == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch in valid_loader:
                        ...
                        topics = torch.stack(topics, dim=1).to(device)
                        topic_emb = val_batch['topic_embedding']
                        topic_emb = torch.stack(topic_emb, dim=1).to(device)
                        pred = model(topics.float(), topic_emb.float())
    ...
    save_metrics(save_path + model_name + 'metrics.pt', train_loss_list, valid_loss_list, global_steps_list, log_file)
    print('Finished Training!', file=log_file)


parser = argparse.ArgumentParser(description='Training MLP classification using topic vectors + topic embeddings')
...
parser.add_argument('--data_path', type=str, default='', help='path to the train-valid sets')
...
model_name = "DTD-DTE_"+str(args.num_topics)+"topics_" + \
             str(args.topic_dim) + "topicDim_" + str(args.num_epochs)+"epochs_" + args.model_id

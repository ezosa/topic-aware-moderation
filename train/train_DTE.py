...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for train_batch in train_loader:
            ...
            topics = train_batch['topic_embedding']
            ...
            if global_step % eval_every == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch in valid_loader:
                        ...
                        topics = val_batch['topic_embedding']
                        topics = torch.stack(topics, dim=1).to(device)
                        pred = model(topics.float())
    ...
    save_metrics(save_path + 'metrics.pt', train_loss_list, valid_loss_list, global_steps_list, log_file)
    print('Finished Training!', file=log_file)


parser = argparse.ArgumentParser(description='Training MLP classification using topic embeddings')
...
model_name = "DTE_"+str(args.num_topics)+"topics_"+str(args.topic_dim) + "topicDim_" + \
             str(args.hidden_size) + "hidden_" + str(args.num_epochs)+"epochs_" + args.model_id
...
model = MLP(args.topic_dim, args.hidden_size)
model.to(device)
print(model)

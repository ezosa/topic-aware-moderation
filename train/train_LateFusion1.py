...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for train_batch in train_loader:
            ...
            pred = model(text, text_len, topics.float())
    ...
    save_metrics(save_path + model_name + '_metrics.pt', train_loss_list, valid_loss_list, global_steps_list, log_file)
    print('Finished Training!', log_file)


parser = argparse.ArgumentParser(description='Training Late Fusion 1: text + topic vectors')
...
parser.add_argument('--mlp_hidden_size', type=int, default=200, help='hidden size')
...
parser.add_argument('--batch_size', type=int, default=800, help='batch_size')
...
model_name = "LF1_"+str(args.num_topics)+"topicVectors_" + str(args.topic_dim) + "topicDim_" + \
...
model = LSTM_MLP_Ensemble(mlp_args=mlp_args,
                          lstm_args=lstm_args)
model.to(device)
print(model)

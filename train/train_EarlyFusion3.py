...
from models.LSTMEnsembleEncoder2 import LSTMEncoder2
...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for train_batch in train_loader:
            ...
            pred = model(text, text_len, topics.float(), topic_embedding.float()).to(device)
...
parser = argparse.ArgumentParser(description='Training Early Fusion 3: text + topic vectors + topic embeddings')
...
lstm_args['topic_emb_dim'] = args.topic_dim

mlp_args = {}
mlp_args['hidden_size'] = args.mlp_hidden_size


model_name = "EF3_" + str(args.num_topics) + "topicVectors_" + \
             str(args.topic_dim) + "topicDim_" + str(args.lstm_hidden_size) + "LSTMhidden_" + \
             str(args.mlp_hidden_size) + "MLPhidden_" + str(args.num_epochs) + "epochs_" + args.model_id
if len(args.pretrained_emb) > 0:
...
model = LSTMEncoder2(lstm_args=lstm_args,
                     mlp_args=mlp_args)
model.to(device)
print(model)
